...
if(size(handles.chain_code))
    ...
    if(numofharmoinc>handles.numofharmoinc)
        ...
        [x_,~,~,~,~] = fourier_approx_norm_modify(handles.chain_code, numofharmoinc, 400, 0, mode,option);
        ...
        [x_,~,~,~,~] = fourier_approx_norm_modify(handles.chain_code, numofharmoinc, 400, 1, mode,option);
        x=[x_; x_(1,1) x_(1,2)];
...
[~,a,b,c,d] = fourier_approx_norm_modify(handles.chain_code, numofharmoinc, 1000, 1, 0,option);
t=[a b c d];
Hs=reshape(t',1,[]);
