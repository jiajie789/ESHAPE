...
if(handles.isaxis1dataready == 1)
    ...
    handles.daseboard = contour_extraction_func(handles.axes1graydata,methodstr,handles.threshvalue);
...
%        fprintf(fid,'\n');
    ...
    [chain_code,oringin,endpoint] = gui_chain_code_func(line_bo,boundary(1,:));%user_point
    size(chain_code)
    axes(handles.axes3);
...
if(islogical(handles.axes1graydata))
    ...
    handles.axes1graydata = enhancement_func(handles.axes1graydata);
end
axes(handles.axes1);
...
if(handles.isaxis1dataready == 1)
    ...
    handles.erzhi=corrosion_func(handles.erzhi,circle);
...
if(handles.isaxis3dataready == 1)
    ...
    handles.erzhi=dilation_func(handles.erzhi);
    axes(handles.axes3);
%     handles.axes3handle = imshow(handles.erzhi);%
    axis3viewdata = handles.erzhi(handles.axis1_rowst+1:handles.axis1_rowst+handles.axis1_nrows,handles.axis1_colst+1:handles.axis1_colst+handles.axis1_ncols);
    handles.axes3handle = imshow(axis3viewdata);
...
%         disp('The input image is a binary image.');
        temp = binarize_func(handles.axes1graydata);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if(dimen_size(end) == 3)
        handles.axes1graydata = grayscale_func(mask1);% to grayscale
    else
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.imdata=inverted_colors_func(handles.imdata);
axes(handles.axes6);
handles.axes6handle = imshow(handles.imdata);
guidata(hObject, handles);
...
[~,a,b,c,d] = fourier_approx_norm_modify(handles.chain_code, numofharmoinc, 1000, 1, 0);
Hs = [a;b;c;d];
